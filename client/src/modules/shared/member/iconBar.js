    ...
    calculateIcon() {
        let view = this.props.members.slice(0, this.props.maxIcons - 1).map((members) =>
            <span id={"ID"+members}>
              ...
              <UncontrolledTooltip placement="bottom" target={"ID"+members} delay={0}> {members} </UncontrolledTooltip>
            </span>)
        ...
        if (this.props.members.length > this.props.maxIcons) {
            ...
            view.push(
              <span id="additionalMembers">
                ...
                <UncontrolledTooltip placement="bottom" target="additionalMembers" delay={0} > {restMember} </UncontrolledTooltip>
              </span>
            )
        }
        return view;
