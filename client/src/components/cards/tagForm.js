...
class TagForm extends React.Component {
        ...
        if (this.state.showSearch) {
            return (
                <div>
                    <Row>
                        <Col>
                            {this.state.search} <Button onClick={() => this.addTag(this.state.search)}
                                                        color="link">Add</Button>
                        </Col>
                    </Row>
    ...
    render() {
        return (
            <div>
                <Input type="text" id="search" value={this.state.search} placeholder="Bitte Tag eingeben."
                       onChange={this.handleChange}></Input>
                {
                    this.state.tags.map((tag) =>
                        <Badge className="mr-1 mt-1">{tag} <Button onClick={() => this.handleDelete(tag)} color="link"
                                                                   className="p-0 text-white">x</Button></Badge>
                    )
                }
                {this.getTagList()}
            </div>
